...
/**
 * This manager implements the route version of the algorithm to ensure the
 * module order property.
 */
public class RouteModuleOrderManager implements ConcurrencyManager {	
	...
	private static class Computation {
		...
		// The alive CORs in each critical path
		public HashMap<Long, HashMap<ServiceCallOrResponse, Integer>> pathAliveCORs = new HashMap<Long, HashMap<ServiceCallOrResponse, Integer>>();
	 ...
	 *            scheduler
	 */
	public RouteModuleOrderManager(int nbThread) {
		super();
	...
	public void addInternalTask(AtomicTask task)
		...
		if (task.getCOR().isCritical) {
			if (c.currentTask.pathID == 0) {
				...
				// Maintain Alive COR
				Iterator<ServiceCallOrResponse> itCOR = dependenciesAndInfluences
						.get(task.getCOR()).iterator();
				while (itCOR.hasNext()) {
					ServiceCallOrResponse corTmp = itCOR.next();
					int counter = c.pathAliveCORs.get(c.currentTask.pathID).get(corTmp);

					c.pathAliveCORs.get(c.currentTask.pathID).put(corTmp, counter + 1);
				}
	...
	private void initiateCriticalPath(AtomicTask task,
		...
		synchronized (pathAccessRights) {
			...
			// computation and update the aliveCors
			HashMap<ServiceCallOrResponse, Integer> pathAliveCORs = new HashMap<ServiceCallOrResponse, Integer>();
			...
			while (itCOR.hasNext()) {
				...
				pathAliveCORs.put(corTmp, 1);
			}
			c.pathAliveCORs.put(task.pathID, pathAliveCORs);
	...
	private void taskExecuted(AtomicTask task, Computation c) {
		if (task.getCOR().equals(ServiceCallOrResponse.nullCOR)) {
			...
			synchronized (pathAccessRights) {
				Iterator<ServiceCallOrResponse> itInfCor = dependenciesAndInfluences
					.get(task.getCOR()).iterator();
				while (itInfCor.hasNext()) {
					ServiceCallOrResponse corTmp = itInfCor.next();
					if (!corTmp.equals(ServiceCallOrResponse.nullCOR)) {
						int counter = c.pathAliveCORs.get(task.pathID).get(corTmp) - 1;

						if (counter > 0) {
							c.pathAliveCORs.get(task.pathID).put(corTmp, counter);
						} else {
							synchronized (corTmp) {
								pathAccessRights.get(corTmp).remove(task.pathID);
								corTmp.notifyAll();
							}
						}
					}
