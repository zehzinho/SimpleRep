...
import seqSamoa.protocols.order.ProtocolFIFOOrder;
...
public class FIFOMain extends ProtocolStack {
	...
	Service.Listener fifoListener;

	public FIFOMain(PID myself, TList processes, SamoaScheduler scheduler,
		...
		try {
			fifo = new FIFO("fifo", this);
		} catch (AlreadyExistingServiceException ex) {
			throw new RuntimeException("This could not happen!");
		}

		fifoListener = fifo.new Listener(this, new LinkedList<ServiceCallOrResponse>()) {
			public void evaluate(FIFOResponseParameters infos,
				...
				System.out.println("FIFODeliver: "
						+ msg.removeFirst()
						+ " TEMPS ECOULE: "
		...
		try {
			new ProtocolFIFOOrder(new String("FIFO"), this, fifo, rpt2pt);
	...
	public void call(Transportable message, TList dests) {
		FIFOCallParameters params = new FIFOCallParameters(dests);

		fc.enter();
		long cid = fifo.externalCall(params, new Message(message, fifoListener));
	...
	public static void main(String args[]) {
		...
		FIFOMain stack = new FIFOMain(myself, processes, new SamoaScheduler(
				manager), new SamoaFlowControl(100), null, null);
		try {
			stack.init();
		...
		while (open) {
			...
			System.out.println("|             SAMBA TEST             |");
			System.out.println("+------------------------------------+");
			System.out.println("|            FIFO_PROTOCOL           |");
			System.out.println("|------------------------------------|");
			System.out.println("LOCALHOST.ip          = " + myself.ip);
			System.out.println("LOCALHOST.port        = " + port);
